...
final String modif = "brain_soft.png";
...
void draw() {
  ...
  for (int i=0; i<STEPS && learning; ++i) {
    ...
    double[] in = crossp2a(cin, src.pixels[index(x,y-1,max)], src.pixels[index(x,y+1,max)],
  ...
  if (infering || realtime) {
    ...
    for (int y=0; y<nSrc.height; ++y) {
      for (int x=0; x<nSrc.width; ++x) {
        ...
        double[] out = nn.infer(crossp2a(in, nSrc.pixels[index(x,y-1,max)], nSrc.pixels[index(x,y+1,max)],
...
double[] crossp2a(color c, color hn, color hs, color hw, color he, int x, int y) {
  ...
  a[15] = map(x,0,width/2,0,1);
  a[16] = map(y,0,height/2,0,1);
  return a;
}
